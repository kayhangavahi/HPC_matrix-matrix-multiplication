...
void dgemm_1B (double *a, double *b, double *c, int n,int B) {
	...
	int i,j,k,i1,j1,k1;
	register double c00,c10,c01,c11,a00,a10,a01,a11,b00,b10,b01,b11;
	for(i=0; i<n; i+=B)
		for(j=0; j<n; j+=B)
			for(k=0; k<n; k+=B)
...
void dgemm_2B (double *a, double *b, double *c, int n,int B) {
	...
	int i,j,k,i1,j1,k1;
	register double rc1,rc2,rc3,rc4,ra1,ra2,ra3,ra4,rb1,rb2,rb3,rb4;
	for(j=0; j<n; j+=B)
		for(i=0; i<n; i+=B)
			for(k=0; k<n; k+=B)
				for(j1=j; j1<j+B; j1+=2)
...
void dgemm_3B (double *a, double *b, double *c, int n,int B) {
	...
	int i,j,k,i1,j1,k1;
	register double a00,a11,a10,a01,b00,b11,b10,b01;
	for(k=0; k<n; k+=B)
		for(i=0; i<n; i+=B)
			for(j=0; j<n; j+=B)
				for(k1=k; k1<k+B; k1+=2)
					for(i1=i; i1<i+B; i1+=2){
						...
						for(j1=j; j1<j+B; j1+=2){
							...
							c[(i1+0)*n+(j1+0)]+= a00 * b00 + a01 * b10;
						
							c[(i1+0)*n+(j1+1)]+= a00 * b01 + a01 * b11;
				
							c[(i1+1)*n+(j1+0)]+= a10 * b00 + a11 * b10;
